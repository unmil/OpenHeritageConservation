...
//		console.log(jsonFeautres[4]['properties']);
	...
	vector_layer.on("postcompose", function () {
		...
		vectorlayer_features.forEach(function (afeature) {
			var opacity=1;
			var color="#ffffff";
			if(afeature.get("Type"=="Efflorescence")){
				opacity=0.4;
			}
			...
			var src;
			switch (afeature.get("Type")) {
			case 'Wood':
				src = 'images/icons/wood2.png';
				break;
			case 'Graffiti':
				src = 'images/icons/graffiti.png';
				break;
			case 'Nail':
				src = 'images/icons/nail.png';
				break;
			case 'Incised Line':
				src = 'images/icons/incizedline.png';
				break;
			default:
				src = 'images/icons/incizedline.png';
			};
			...
			afeature.setStyle(new ol.style.Style({
				...
				/*
                    fill: new ol.style.Fill({
                        color: colorbrewer[afeature.get("Color")],
                        opacity: 0.2
                    }),
                    stroke: new ol.style.Stroke({
                        color: colorbrewer[afeature.get("Color")],
...
*/
	...
	function addDrawInteraction() {
		...
		draw_interaction.on('drawend', function (event) {
			...
			if (getX(coordinate) >= 0 && getX(coordinate) <= 22346 && getY(coordinate) <= 0 && getY(coordinate) >= (-31438)) {
				...
				adjust($("div.ol-overlaycontainer-stopevent div:first-child"));
	...
	submit.onclick = function () {
		if ($('#type').val() === "Select") {
			...
			if (haveColorAttribute($("#type").val())) {
				properties.Color = $('#color').val();
			}
			if (haveActorAttribute($("#type").val())) {
...
*/
	...
	function haveColorAttribute(type) {
		var have = $.inArray(type, typeHasColor) > -1 ? true : false;
		console.log(have);
		return have;
	}

	function haveActorAttribute(type) {
		var have = $.inArray(type, typeHasActor) > -1 ? true : false;
		console.log(have);
		return have;
	}
